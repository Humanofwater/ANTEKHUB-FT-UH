// File: scripts/test-api-referensi-jabatan.js
// Tujuan: Otomatisasi testing CRUD referensi_jabatan.controller.js (master table, tanpa pagination)
// Jalankan: node scripts/test-api-referensi-jabatan.js
...
const outFile = path.join(OUT_DIR, `test-referensi-jabatan-results-${stamp}.json`);

const seededSuperAdmin = { username: 'superadmin_7843', password: 'SAdmin#7843' };

const results = [];
let TOKEN = null;
let CREATED = []; // simpan uuid jabatan yang dibuat
let CREATED_PAYLOADS = []; // simpan payload untuk validasi filter
...
function makeCreatePayload(i) {
  const now = Date.now();
  const jab = `Jabatan Uji ${i}-${now}`;
  const slg = `jabatan-uji-${i}-${now}`;
  const alias = i % 2 === 0 ? ['Head', 'Lead'] : ['Manager', 'Supervisor'];
  return { jabatan: jab, slug: slg, alias_list: alias };
}

function makeBadPayload_missingRequired() {
  return {
    // jabatan hilang (required)
    slug: `slug-tanpa-jabatan-${Date.now()}`,
    alias_list: ['X', 'Y']
  };
}
...
function makeBadPayload_invalidSchema() {
  return {
    jabatan: '', // required non-empty
    slug: 'x'.repeat(300), // > 255
    alias_list: 'bukan-array' // harus array of string
  };
}

// ====== TESTS ======
async function t1_create_ok() {
  const name = 'RefJabatan: create OK (2 rows)';
  const createdLocal = [];
  const payloadLocal = [];
  for (let i = 1; i <= 2; i++) {
    ...
    payloadLocal.push(body);
  }
  CREATED = createdLocal;
  CREATED_PAYLOADS = payloadLocal;
}

async function t1_create_error_missing_required() {
  const name = 'RefJabatan: create ERROR missing required fields';
  const req = {
    method: 'POST',
    url: `${API}/referensi-jabatan`,
    headers: { ...authHeader(), 'Content-Type': 'application/json' },
...
async function t1_create_error_invalid_schema() {
  const name = 'RefJabatan: create ERROR invalid schema';
  const req = {
    method: 'POST',
    url: `${API}/referensi-jabatan`,
    headers: { ...authHeader(), 'Content-Type': 'application/json' },
...
async function t2_get_list_default() {
  const name = 'RefJabatan: get list default (no pagination)';
  const req = { method: 'GET', url: `${API}/referensi-jabatan`, headers: authHeader() };
  const res = await http(req);
...
async function t2_get_list_filters() {
  // search by jabatan/slug
  const searchTerm = encodeURIComponent('Jabatan Uji');
  const req1 = { method: 'GET', url: `${API}/referensi-jabatan?search=${searchTerm}`, headers: authHeader() };
  const res1 = await http(req1);
  record('RefJabatan: filter search', req1, res1, null, '200');
  assert(res1.status === 200, `Search harus 200, dapat ${res1.status}`);

  // filter slug (multi) — gunakan slug dari data yang dibuat
  const slgA = CREATED_PAYLOADS[0].slug;
  const slgB = CREATED_PAYLOADS[1].slug;
  const req2 = { method: 'GET', url: `${API}/referensi-jabatan?slug=${encodeURIComponent(slgA)}&slug=${encodeURIComponent(slgB)}`, headers: authHeader() };
  const res2 = await http(req2);
  record('RefJabatan: filter slug multi', req2, res2, null, '200');
  assert(res2.status === 200, `Filter slug harus 200, dapat ${res2.status}`);

  // filter alias (contains 1 alias) — controller pakai Op.contains untuk setiap alias
  const oneAlias = CREATED_PAYLOADS[0].alias_list[0]; // contoh: 'Manager' atau 'Head'
  const req3 = { method: 'GET', url: `${API}/referensi-jabatan?alias=${encodeURIComponent(oneAlias)}`, headers: authHeader() };
  const res3 = await http(req3);
  record('RefJabatan: filter alias contains(1)', req3, res3, null, '200');
  assert(res3.status === 200, `Filter alias harus 200, dapat ${res3.status}`);
}

async function t3_get_detail_ok() {
  const name = 'RefJabatan: get detail OK';
  const uuid = CREATED[0];
  const req = { method: 'GET', url: `${API}/referensi-jabatan/${uuid}`, headers: authHeader() };
  const res = await http(req);
...
async function t3_get_detail_not_found() {
  const name = 'RefJabatan: get detail NOT FOUND';
  const req = { method: 'GET', url: `${API}/referensi-jabatan/aaaaaaaa-bbbb-cccc-dddd-eeeeeeeeeeee`, headers: authHeader() };
  const res = await http(req);
...
async function t4_update_ok() {
  const name = 'RefJabatan: update OK (jabatan + alias_list)';
  const uuid = CREATED[0];
  const body = { jabatan: `Jabatan Uji Updated ${Date.now()}`, alias_list: ['Head', 'Manager', 'Director'] };
  const req = {
    method: 'PATCH',
    url: `${API}/referensi-jabatan/${uuid}`,
    headers: { ...authHeader(), 'Content-Type': 'application/json' },
    body
  };
  const res = await http(req);
...
async function t4_update_error_invalid_schema() {
  const name = 'RefJabatan: update ERROR invalid schema';
  const uuid = CREATED[0];
  const body = { alias_list: 'salah_tipe', jabatan: '' }; // alias_list bukan array & jabatan kosong
  const req = {
    method: 'PATCH',
    url: `${API}/referensi-jabatan/${uuid}`,
    headers: { ...authHeader(), 'Content-Type': 'application/json' },
...
async function t5_delete_not_found() {
  const name = 'RefJabatan: delete NOT FOUND';
  const req = { method: 'DELETE', url: `${API}/referensi-jabatan/aaaaaaaa-bbbb-cccc-dddd-eeeeeeeeeeee`, headers: authHeader() };
  const res = await http(req);
  record(name, req, res, null, '404/400');
...
async function t5_delete_ok() {
  const name = 'RefJabatan: delete OK';
  const uuid = CREATED[1]; // hapus yang kedua
  const req = { method: 'DELETE', url: `${API}/referensi-jabatan/${uuid}`, headers: authHeader() };
  const res = await http(req);
  record(name, req, res, null, '200');
...
async function t6_delete_all_ok() {
  const name = 'RefJabatan: delete ALL OK';
  const req = { method: 'DELETE', url: `${API}/referensi-jabatan/__all`, headers: authHeader() };
  const res = await http(req);
  record(name, req, res, null, '200');
