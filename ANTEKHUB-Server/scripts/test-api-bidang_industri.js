// File: scripts/test-api-bidang-industri.js
// Tujuan: Otomatisasi testing CRUD bidang_industri.controller.js (master table, tanpa pagination)
// Jalankan: node scripts/test-api-bidang-industri.js
...
const outFile = path.join(OUT_DIR, `test-bidang-industri-results-${stamp}.json`);

const seededSuperAdmin = { username: 'superadmin_7843', password: 'SAdmin#7843' };

const results = [];
let TOKEN = null;
let CREATED = []; // simpan uuid yang dibuat
...
function makeCreatePayload(i) {
  return {
    kode: i % 2 === 0 ? `FIN-${Date.now()}` : `HEALTH-${Date.now()}`,
    nama: `Bidang Uji ${i}-${Date.now()}`,
    deskripsi: i % 2 === 0 ? 'Sektor keuangan & fintech' : 'Sektor kesehatan & biotek',
    active: i % 2 === 1
  };
}
...
function makeBadPayload_missingRequired() {
  return {
    // nama hilang (required)
    kode: 'NOPENAME',
    deskripsi: 'Data tanpa nama',
    active: true
  };
}
...
function makeBadPayload_invalidSchema() {
  return {
    kode: 'X'.repeat(200),  // > 50? (controller batasi 50 untuk kode; jika >50 -> invalid)
    nama: '',               // required non-empty
    deskripsi: 123,         // harus string/null
    active: 'aktif'         // harus boolean
  };
}

// ====== TESTS ======
async function t1_create_ok() {
  const name = 'BidangIndustri: create OK (2 rows)';
  const createdLocal = [];
  for (let i = 1; i <= 2; i++) {
    const req = {
      method: 'POST',
...
async function t1_create_error_missing_required() {
  const name = 'BidangIndustri: create ERROR missing required fields';
  const req = {
    method: 'POST',
    url: `${API}/bidang-industri`,
    headers: { ...authHeader(), 'Content-Type': 'application/json' },
...
async function t1_create_error_invalid_schema() {
  const name = 'BidangIndustri: create ERROR invalid schema';
  const req = {
    method: 'POST',
    url: `${API}/bidang-industri`,
    headers: { ...authHeader(), 'Content-Type': 'application/json' },
...
async function t2_get_list_default() {
  const name = 'BidangIndustri: get list default (no pagination)';
  const req = { method: 'GET', url: `${API}/bidang-industri`, headers: authHeader() };
  const res = await http(req);
...
async function t2_get_list_filters() {
  // search by nama/kode/deskripsi
  const req1 = { method: 'GET', url: `${API}/bidang-industri?search=Bidang%20Uji`, headers: authHeader() };
  const res1 = await http(req1);
  record('BidangIndustri: filter search', req1, res1, null, '200');
  assert(res1.status === 200, `Search harus 200, dapat ${res1.status}`);

  // filter active=true
  const req2 = { method: 'GET', url: `${API}/bidang-industri?active=true`, headers: authHeader() };
  const res2 = await http(req2);
  record('BidangIndustri: filter active=true', req2, res2, null, '200');
  assert(res2.status === 200, `Filter active=true harus 200, dapat ${res2.status}`);

  // filter kode (multi)
  const req3 = { method: 'GET', url: `${API}/bidang-industri?kode=FIN&kode=HEALTH`, headers: authHeader() };
  const res3 = await http(req3);
  record('BidangIndustri: filter kode multi', req3, res3, null, '200');
  assert(res3.status === 200, `Filter kode multi harus 200, dapat ${res3.status}`);
}

async function t3_get_detail_ok() {
  const name = 'BidangIndustri: get detail OK';
  const uuid = CREATED[0];
  const req = { method: 'GET', url: `${API}/bidang-industri/${uuid}`, headers: authHeader() };
  const res = await http(req);
...
async function t3_get_detail_not_found() {
  const name = 'BidangIndustri: get detail NOT FOUND';
  const req = { method: 'GET', url: `${API}/bidang-industri/aaaaaaaa-bbbb-cccc-dddd-eeeeeeeeeeee`, headers: authHeader() };
  const res = await http(req);
...
async function t4_update_ok() {
  const name = 'BidangIndustri: update OK (nama + active)';
  const uuid = CREATED[0];
  const body = { nama: `Bidang Uji Updated ${Date.now()}`, active: true };
  const req = {
    method: 'PATCH',
    url: `${API}/bidang-industri/${uuid}`,
    headers: { ...authHeader(), 'Content-Type': 'application/json' },
    body
  };
  const res = await http(req);
...
async function t4_update_error_invalid_schema() {
  const name = 'BidangIndustri: update ERROR invalid schema';
  const uuid = CREATED[0];
  const body = { active: 'ya', kode: 'X'.repeat(200) }; // tipe salah & panjang kode berlebih
  const req = {
    method: 'PATCH',
    url: `${API}/bidang-industri/${uuid}`,
    headers: { ...authHeader(), 'Content-Type': 'application/json' },
...
async function t5_delete_not_found() {
  const name = 'BidangIndustri: delete NOT FOUND';
  const req = { method: 'DELETE', url: `${API}/bidang-industri/aaaaaaaa-bbbb-cccc-dddd-eeeeeeeeeeee`, headers: authHeader() };
  const res = await http(req);
  record(name, req, res, null, '404/400');
...
async function t5_delete_ok() {
  const name = 'BidangIndustri: delete OK';
  const uuid = CREATED[1]; // hapus yang kedua
  const req = { method: 'DELETE', url: `${API}/bidang-industri/${uuid}`, headers: authHeader() };
  const res = await http(req);
  record(name, req, res, null, '200');
...
async function t6_delete_all_ok() {
  const name = 'BidangIndustri: delete ALL OK';
  const req = { method: 'DELETE', url: `${API}/bidang-industri/__all`, headers: authHeader() };
  const res = await http(req);
  record(name, req, res, null, '200');
