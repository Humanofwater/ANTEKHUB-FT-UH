// File: scripts/test-api-bank.js
// Tujuan: Otomatisasi testing CRUD bank.controller.js (master table, tanpa pagination)
// Jalankan: node scripts/test-api-bank.js
...
const outFile = path.join(OUT_DIR, `test-bank-results-${stamp}.json`);

const seededSuperAdmin = {
  username: "superadmin_7843",
  password: "SAdmin#7843",
};

const results = [];
let TOKEN = null;
let CREATED = []; // simpan uuid bank yang dibuat
...
function makeCreatePayload(i) {
  return {
    nama: `Bank Uji ${i}-${Date.now()}`, // unik
    kategori: i % 2 === 0 ? "BANK" : "E-WALLET",
  };
}
...
// Payload salah: validation gagal (tipe/panjang salah)
function makeBadPayload_invalidSchema() {
  return {
    nama: "", // kosong → invalid
    kategori: "X".repeat(64), // > 50 → invalid oleh Joi
    // atau kategori: 123
  };
}
...
async function t1_create_ok() {
  const name = "Bank: create OK (2 rows)";
  const createdLocal = [];
  for (let i = 1; i <= 2; i++) {
    const req = {
      method: "POST",
    ...
    assert(
      res.data && res.data.data && res.data.data.uuid,
      "Response tidak mengandung data.uuid"
    );
...
async function t1_create_error_missing_required() {
  const name = "Bank: create ERROR missing required fields";
  const req = {
    method: "POST",
    url: `${API}/bank`,
    headers: { ...authHeader(), "Content-Type": "application/json" },
  ...
  assert(
    ...
    `Missing required harus 400/422, dapat ${res.status}`
  );
}

async function t1_create_error_invalid_schema() {
  const name = "Bank: create ERROR invalid schema";
  const req = {
    method: "POST",
    url: `${API}/bank`,
    headers: { ...authHeader(), "Content-Type": "application/json" },
  ...
  assert(
    [400, 422].includes(res.status),
    `Invalid schema harus 400/422, dapat ${res.status}`
  );
}

// LIST (tanpa pagination)
async function t2_get_list_default() {
  const name = "Bank: get list default (no pagination)";
  const req = { method: "GET", url: `${API}/bank`, headers: authHeader() };
  const res = await http(req);
...
async function t2_get_list_filters() {
  ...
  const req1 = {
    method: "GET",
    url: `${API}/bank?search=Bank%20Uji`,
    headers: authHeader(),
  };
  const res1 = await http(req1);
  record("Bank: filter search by nama", req1, res1, null, "200");
  assert(res1.status === 200, `Search harus 200, dapat ${res1.status}`);

  // filter kategori (multi)
  const req2 = {
    method: "GET",
    url: `${API}/bank?kategori=BANK&kategori=E-WALLET`,
    headers: authHeader(),
  };
  const res2 = await http(req2);
  record("Bank: filter kategori=BANK&E-WALLET", req2, res2, null, "200");
  assert(
    res2.status === 200,
    `Filter kategori harus 200, dapat ${res2.status}`
  );
}

async function t3_get_detail_ok() {
  const name = "Bank: get detail OK";
  ...
  assert(
    res.data && res.data.data && res.data.data.uuid === uuid,
    "Detail UUID tidak cocok"
  );
}

async function t3_get_detail_not_found() {
  const name = "Bank: get detail NOT FOUND";
  const req = {
    method: "GET",
    url: `${API}/bank/aaaaaaaa-bbbb-cccc-dddd-eeeeeeeeeeee`,
    headers: authHeader(),
  };
  const res = await http(req);
  ...
  assert(
    ...
    `Not-found harus 404/400, dapat ${res.status}`
  );
}

async function t4_update_ok() {
  const name = "Bank: update OK (nama + kategori)";
  const uuid = CREATED[0];
  const body = { nama: `Bank Uji Updated ${Date.now()}`, kategori: "BANK" };
  const req = {
    method: "PATCH",
    url: `${API}/bank/${uuid}`,
    headers: { ...authHeader(), "Content-Type": "application/json" },
...
async function t4_update_error_invalid_schema() {
  const name = "Bank: update ERROR invalid schema";
  const uuid = CREATED[0];
  const body = { nama: "", kategori: "X".repeat(100) }; // invalid: nama kosong & kategori > 50
  const req = {
    method: "PATCH",
    url: `${API}/bank/${uuid}`,
    headers: { ...authHeader(), "Content-Type": "application/json" },
  ...
  assert(
    [400, 422].includes(res.status),
    `Invalid schema seharusnya 400/422, dapat ${res.status}`
  );
}

async function t5_delete_not_found() {
  const name = "Bank: delete NOT FOUND";
  const req = {
    method: "DELETE",
    url: `${API}/bank/aaaaaaaa-bbbb-cccc-dddd-eeeeeeeeeeee`,
    headers: authHeader(),
  ...
  assert(
    [404, 400].includes(res.status),
    `Delete not-found seharusnya 404/400, dapat ${res.status}`
  );
}

async function t5_delete_ok() {
  const name = "Bank: delete OK";
