// File: scripts/test-api-jenis-institusi.js
// Tujuan: Otomatisasi testing CRUD jenis_institusi.controller.js (master table, tanpa pagination)
// Jalankan: node scripts/test-api-jenis-institusi.js
...
const outFile = path.join(OUT_DIR, `test-jenis-institusi-results-${stamp}.json`);
...
function makeCreatePayload(i) {
  return {
    kode: i % 2 === 0 ? `PT-${Date.now()}` : `YYS-${Date.now()}`,
    nama: `Jenis Institusi Uji ${i}-${Date.now()}`,
    deskripsi: i % 2 === 0 ? 'Perseroan Terbatas' : 'Yayasan',
    active: i % 2 === 1
  };
}
...
async function t1_create_ok() {
  const name = 'JenisInstitusi: create OK (2 rows)';
  const createdLocal = [];
  for (let i = 1; i <= 2; i++) {
    const req = {
      method: 'POST',
...
async function t1_create_error_missing_required() {
  const name = 'JenisInstitusi: create ERROR missing required fields';
  const req = {
    method: 'POST',
    url: `${API}/jenis-institusi`,
    headers: { ...authHeader(), 'Content-Type': 'application/json' },
...
async function t1_create_error_invalid_schema() {
  const name = 'JenisInstitusi: create ERROR invalid schema';
  const req = {
    method: 'POST',
    url: `${API}/jenis-institusi`,
    headers: { ...authHeader(), 'Content-Type': 'application/json' },
...
async function t2_get_list_default() {
  const name = 'JenisInstitusi: get list default (no pagination)';
  const req = { method: 'GET', url: `${API}/jenis-institusi`, headers: authHeader() };
  const res = await http(req);
...
async function t2_get_list_filters() {
  ...
  const req1 = { method: 'GET', url: `${API}/jenis-institusi?search=Jenis%20Institusi%20Uji`, headers: authHeader() };
  const res1 = await http(req1);
  record('JenisInstitusi: filter search', req1, res1, null, '200');
  assert(res1.status === 200, `Search harus 200, dapat ${res1.status}`);

  // filter active=true
  const req2 = { method: 'GET', url: `${API}/jenis-institusi?active=true`, headers: authHeader() };
  const res2 = await http(req2);
  record('JenisInstitusi: filter active=true', req2, res2, null, '200');
  assert(res2.status === 200, `Filter active=true harus 200, dapat ${res2.status}`);

  // filter kode (multi)
  const req3 = { method: 'GET', url: `${API}/jenis-institusi?kode=PT&kode=YYS`, headers: authHeader() };
  const res3 = await http(req3);
  record('JenisInstitusi: filter kode multi', req3, res3, null, '200');
  assert(res3.status === 200, `Filter kode multi harus 200, dapat ${res3.status}`);
}

async function t3_get_detail_ok() {
  const name = 'JenisInstitusi: get detail OK';
  const uuid = CREATED[0];
  const req = { method: 'GET', url: `${API}/jenis-institusi/${uuid}`, headers: authHeader() };
  const res = await http(req);
...
async function t3_get_detail_not_found() {
  const name = 'JenisInstitusi: get detail NOT FOUND';
  const req = { method: 'GET', url: `${API}/jenis-institusi/aaaaaaaa-bbbb-cccc-dddd-eeeeeeeeeeee`, headers: authHeader() };
  const res = await http(req);
...
async function t4_update_ok() {
  const name = 'JenisInstitusi: update OK (nama + active)';
  const uuid = CREATED[0];
  const body = { nama: `Jenis Institusi Updated ${Date.now()}`, active: true };
  const req = {
    method: 'PATCH',
    url: `${API}/jenis-institusi/${uuid}`,
    headers: { ...authHeader(), 'Content-Type': 'application/json' },
    body
  };
  const res = await http(req);
...
async function t4_update_error_invalid_schema() {
  const name = 'JenisInstitusi: update ERROR invalid schema';
  const uuid = CREATED[0];
...
async function t5_delete_not_found() {
  const name = 'JenisInstitusi: delete NOT FOUND';
  const req = { method: 'DELETE', url: `${API}/jenis-institusi/aaaaaaaa-bbbb-cccc-dddd-eeeeeeeeeeee`, headers: authHeader() };
  const res = await http(req);
  record(name, req, res, null, '404/400');
...
async function t5_delete_ok() {
  const name = 'JenisInstitusi: delete OK';
  const uuid = CREATED[1]; // hapus yang kedua
  const req = { method: 'DELETE', url: `${API}/jenis-institusi/${uuid}`, headers: authHeader() };
  const res = await http(req);
  record(name, req, res, null, '200');
...
async function t6_delete_all_ok() {
  const name = 'JenisInstitusi: delete ALL OK';
  const req = { method: 'DELETE', url: `${API}/jenis-institusi/__all`, headers: authHeader() };
  const res = await http(req);
  record(name, req, res, null, '200');
