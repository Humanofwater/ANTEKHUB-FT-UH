// File: src/controllers/jenis_institusi.controller.js
// Tujuan: CRUD master "JenisInstitusi" (tanpa pagination; gaya mengikuti bangsa.controller)
...
exports.add = async (req, res) => {
  ...
  try {
    ...
    const row = await JenisInstitusi.create(
      {
    ...
    await t.commit();
    return res.status(201).json({ message: 'Jenis institusi dibuat', data: toPlain(row) });
  } catch (err) {
    await t.rollback();
    if (err.isJoi) {
    ...
    console.error('jenis_institusi.add error:', err);
    return res.status(500).json({ message: 'Gagal membuat jenis institusi' });
  }
};
...
exports.getAll = async (req, res) => {
  try {
    ...
    const rows = await JenisInstitusi.findAll({
      where,
      order: [['nama', 'ASC']],
    });
    ...
    console.error('jenis_institusi.getAll error:', err);
    return res.status(500).json({ message: 'Gagal mengambil data jenis institusi' });
  }
};
...
exports.getOne = async (req, res) => {
  try {
    ...
    const row = await JenisInstitusi.findOne({ where: { uuid } });
    if (!row) return res.status(404).json({ message: 'Jenis institusi tidak ditemukan' });
    return res.json({ data: toPlain(row) });
  } catch (err) {
    console.error('jenis_institusi.getOne error:', err);
    return res.status(500).json({ message: 'Gagal mengambil detail jenis institusi' });
  }
};
...
exports.updateByUuid = async (req, res) => {
  ...
  try {
    ...
    if (!row) {
      await t.rollback();
      return res.status(404).json({ message: 'Jenis institusi tidak ditemukan' });
    }
    ...
    await t.commit();
    return res.json({ message: 'Jenis institusi diperbarui', data: toPlain(row) });
  } catch (err) {
    await t.rollback();
    if (err.isJoi) {
      return res.status(422).json({
    ...
    console.error('jenis_institusi.update error:', err);
    return res.status(500).json({ message: 'Gagal memperbarui jenis institusi' });
  }
};
...
exports.deleteByUuid = async (req, res) => {
  try {
    ...
    const found = await JenisInstitusi.findAll({ where: { uuid: { [Op.in]: uuids } }, attributes: ['uuid'] });
    if (!found.length) return res.status(404).json({ message: 'Data jenis institusi tidak ditemukan' });

    const deleted = await JenisInstitusi.destroy({ where: { uuid: { [Op.in]: uuids } } });
    return res.json({ message: `Berhasil menghapus ${deleted} data jenis institusi`, deleted, uuids });
  } catch (err) {
    console.error('jenis_institusi.deleteByUuid error:', err);
    return res.status(500).json({ message: 'Gagal menghapus jenis institusi', detail: err.message });
  }
};
...
exports.deleteAll = async (_req, res) => {
  try {
    const del = await JenisInstitusi.destroy({ where: {} });
    return res.json({ message: 'Semua jenis institusi terhapus', deleted: del });
  } catch (err) {
    console.error('jenis_institusi.deleteAll error:', err);
    return res.status(500).json({ message: 'Gagal menghapus semua jenis institusi', detail: err.message });
  }
};
